...
def make_dataset_raw():
    ...
    df = df.replace([np.inf, -np.inf], np.nan).dropna()
    df['1'] = df['1'].apply(int, base=16)
    df['3'] = df['3'].apply(lambda x: int(x.replace(' ', ''), base=16))
    ...
    for i in range(Config.N_INTVL, num_data-1):
        ...
        if Config.isMC:
            ...
            labels.append(1) if 'Attack' in chunk.values else labxels.append(0)
        d1 = chunk['1'].apply(lambda x : id2bit(x))
        d2 = chunk['2'].to_numpy()[:, None]
        d3 = chunk['3'].to_numpy()[:, None]
        if not d1.empty:
            d = np.concatenate((np.stack(d1), d2, d3), axis=1)
            data.append(d)
        else: data.append(np.concatenate((np.zeros((40, 31)), d2, d3),axis=1))
