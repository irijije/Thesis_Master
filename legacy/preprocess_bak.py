...
def make_dataset_lstm():
    ...
    data, _, labels, _  = train_test_split(np.array(data), np.array(labels), test_size=0.01)

    print(data.shape)

    np.save(Config.DATAPATH+"data", np.array(data))
...
def make_dataset_cnn():
    ...
    data, labels = shuffle(np.array(data)[Config.N_INTVL:], np.array(labels))

    np.save(Config.DATAPATH+"data2", np.array(data))
    np.save(Config.DATAPATH+"labels2", np.array(labels))
